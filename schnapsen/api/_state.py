...
class State:
    ...
    def next(self,
        ...
        if move[1] is not None:
            ...
            if state.__leads_turn:
                raise RuntimeError(
                    "Marriage was attempted to be melded by non-leading player")

            # Update perspective since an additional card is revealed by the player who performs a marriage.
            state.__deck.add_to_perspective(util.other(
                state.whose_turn()), move[1], "P" + str(state.whose_turn()) + "H")
        ...
        if not state.__leads_turn:
            ...
            state.__deck.add_to_perspective(
                state.whose_turn(), trick[other-1], "P" + str(other) + "H")
            return state
    ...
    def moves(self):
        ...
        # If the game is in phase 2 and it's not the leader's turn, then some constraints apply
            ...
            same_suit_hand = [card for card in hand if Deck.get_suit(
                card) == Deck.get_suit(opponent_card)]
            playable_cards = None

            if len(same_suit_hand) > 0:
                same_suit_hand_higher = [
                    card for card in same_suit_hand if card < opponent_card]
                ...
                trump_hand = [card for card in hand if Deck.get_suit(
                    card) == self.__deck.get_trump_suit()]
                if len(trump_hand) > 0:
        ...
        if self.whose_turn() == self.leader():
            ...
            if self.__deck.can_exchange(self.whose_turn()):
                possible_moves.append(
                    (None, self.__deck.get_trump_jack_index()))

            possible_mariages = self.__deck.get_possible_mariages(
                self.whose_turn())
    ...
    def clone(self, signature=None):
        ...
        """
        state = State(self.__deck.clone(signature), self.__player1s_turn, self.__p1_points,
                      self.__p2_points, self.__p1_pending_points, self.__p2_pending_points)
    ...
    def __evaluate_trick(self, trick):
        ...
        if len(trick) != 2:
            raise RuntimeError(
                "Incorrect trick format. List of length 2 needed.")
    ...
    def convert_to_json(self):
        if self.__signature is not None:
            raise RuntimeError(
                "Cannot convert partial information state to JSON")
    ...
    def load_from_json(dict):
        ...
        state = State(Deck.load_from_json(dict['deck']), dict['player1s_turn'], dict['p1_points'],
                      dict['p2_points'], dict['p1_pending_points'], dict['p2_pending_points'])
